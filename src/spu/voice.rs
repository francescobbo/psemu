...
impl AdsrEnvelope {
    ...
    pub fn write_high(&mut self, value: u16) {
        self.settings =
            (self.settings & 0x0000_FFFF) | (u32::from(value) << 16);
    }
    ...
    fn check_for_phase_transition(&mut self) {
        ...
        if self.phase == AdsrPhase::Decay
            && (self.level as u16) <= self.sustain_level()
        {
    ...
    pub fn clock(&mut self) {
        ...
        let shift = if direction == Direction::Increasing
        ...
        self.level = match direction {
            ...
            Direction::Decreasing => new_level.clamp(0, i16::MAX.into()) as i16,
        };
    }
}
