...
impl Rom {
    ...
    pub fn read(&self, address: u32, size: usize) -> u32 {
        // Adjust the address to the ROM base
    ...
    fn read16(&self, address: u32) -> u16 {
        if address as usize >= ROM_SIZE - 1 {
            panic!("[Rom] Out of bounds read at {address:#X}");
        }
    ...
    fn read32(&self, address: u32) -> u32 {
        if address as usize >= ROM_SIZE - 3 {
            panic!("[Rom] Out of bounds read at {address:#X}");
        }
