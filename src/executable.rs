...
impl Executable {
    ...
    pub fn load_into(&self, cpu: &mut crate::cpu::Cpu) {
        ...
        let start_address = self.header.load_address & 0x1fff_ffff;
        for (i, byte) in self.code.iter().enumerate() {
            cpu.write_memory(start_address + i as u32, *byte as u32, 1)
                .expect("Failed to write to memory");
        }

        // Zero out the BSS section
        for i in self.header.bss_start..(self.header.bss_start + self.header.bss_size) {
            cpu.write_memory(i, 0, 1)
                .expect("Failed to write to memory");
        }
    }
}
